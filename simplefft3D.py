...
def process(connection, config, metadata):
    ...
    # Discard phase correction lines and accumulate lines until "ACQ_LAST_IN_REPETITION" is set
    for group in conditionalGroups(connection, lambda acq: not acq.is_flag_set(ismrmrd.ACQ_IS_PHASECORR_DATA), lambda acq: acq.is_flag_set(ismrmrd.ACQ_LAST_IN_REPETITION)):
...
def process_group(group, config, metadata):
    ...
    for acq in group:
        ...
        if (lin < data.shape[2]) and (par < data.shape[2]):
            data[:,:,lin,par] = acq.data
            # center line of k-space is encoded in user[5]
            if (np.abs(acq.getHead().idx.kspace_encode_step_1 ==acq.getHead().idx.user[5])) and (np.abs(acq.getHead().idx.kspace_encode_step_2 ==acq.getHead().idx.user[6])):
                rawHead = acq.getHead()
    ...
    if data.shape[-1]>1:
        data = fft.fftshift( data, axes=(-1))
        data = fft.ifft(    data, axis=-1)
        data = fft.ifftshift(data, axes=(-1))
    ...
    if data.shape[-1]>1:
        offset = int((data.shape[2] - metadata.encoding[0].reconSpace.matrixSize.z)/2)
